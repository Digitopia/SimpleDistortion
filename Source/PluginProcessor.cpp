...
#endif
{
    ...
    state->createAndAddParameter("drive", "Drive", "Drive", juce::NormalisableRange<float>(0.f, 1.f, 0.0001), 0.4, nullptr, nullptr);
    state->createAndAddParameter("range", "Range", "Range", juce::NormalisableRange<float>(0.f, 3000.f, 0.0001), 1500.0, nullptr, nullptr);
...
void SimpleDistortionAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        for (int sample = 0; sample < buffer.getNumSamples(); ++sample) {
            ...
            float dry = *channelData;
            
            *channelData *= drive * range;
            
            *channelData = (((((2.f / juce::float_Pi) * atan(*channelData)) * blend) + (dry * (1.f - blend))) / 2.f) * volume;

            // *channelData = (
            //     (2.f * juce::MathConstants<float>::pi * atan(*channelData) * blend) +
            //     (dry * (1.f - blend))
            // ) * volume;
...
juce::AudioProcessorValueTreeState& SimpleDistortionAudioProcessor::getState() {
    return *state;
}
